...
namespace MultiBoost {
	...
	void BanditSingleStumpLearner::init() {
		...
		for( int i=0; i < numColumns; i++ )
		{
			...
			AlphaReal reward = getRewardFromEdge( edge );
	...
	// ------------------------------------------------------------------------------
	AlphaReal BanditSingleStumpLearner::getRewardFromEdge( AlphaReal edge )
	{
		AlphaReal updateWeight = 0.0;
		if ( _updateRule == EDGE_SQUARE ) {
			...
			if ( edge < ( 1.0 - numeric_limits< AlphaReal >::epsilon() ) ) {
				updateWeight = - log(sqrt( 1 - ( edge * edge ) ));
			} else {
				updateWeight = - log( numeric_limits< AlphaReal >::epsilon() );
			}
	...
	AlphaReal BanditSingleStumpLearner::run()
	{
